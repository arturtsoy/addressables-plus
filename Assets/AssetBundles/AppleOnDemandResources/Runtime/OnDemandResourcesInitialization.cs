using System;
using System.IO;
using AssetBundles.AppleOnDemandResources.ResourceProviders;
using UnityEngine;
...
namespace AssetBundles.AppleOnDemandResources
{
    ...
    /// </summary>
    [Serializable]
    public class OnDemandResourcesInitialization : IInitializableObject
    {
        ...
        /// </summary>
        /// <param name="data">The JSON serialized <see cref="OnDemandResourcesInitializationData"/> object</param>
        /// <returns>True to log warnings, otherwise returns false. Default value is true.</returns>
        public bool LogWarnings(string data)
        {
	        OnDemandResourcesInitializationData initializeData = JsonUtility.FromJson<OnDemandResourcesInitializationData>(data);
        ...
        {
            OnDemandResourcesInitializeOperation op = new OnDemandResourcesInitializeOperation();
    ...
    /// </summary>
    public class OnDemandResourcesInitializeOperation : AsyncOperationBase<bool>
    {
        ...
        {
            Addressables.ResourceManager.ResourceProviders.Add(new AppleAssetBundleProvider());
        ...
        private void InitializeBundleToAssetPackMap(string contents)
        {
            ...
            foreach (CustomAssetPackDataEntry entry in customPackData.Entries)
            {
                ...
                {
                    OnDemandResourcesRuntimeData.Instance.BundleNameToAssetPack.Add(bundle, entry);
                }
            }
        }
            ...
            if (location.ResourceType == typeof(IAssetBundleResource))
            {
                ...
                if (OnDemandResourcesRuntimeData.Instance.BundleNameToAssetPack.ContainsKey(bundleName))
                {
                    ...
                    if (OnDemandResourcesRuntimeData.Instance.AssetPackNameToDownloadPath.ContainsKey(assetPackName))
                    {
                        ...
                        // OnDemandResourcesBundleProvider.Provider previously saved the asset pack path.
                        return Path.Combine(OnDemandResourcesRuntimeData.Instance.AssetPackNameToDownloadPath[assetPackName], Path.GetFileName(location.InternalId));
                    }
                }
            }
        ...
        private string EditorTransformFunc(IResourceLocation location)
        {
            if (location.ResourceType == typeof(IAssetBundleResource))
            {
                ...
                if (OnDemandResourcesRuntimeData.Instance.BundleNameToAssetPack.ContainsKey(bundleName))
                {
                    ...
                    {
                        // Load bundle from the 'Assets/AssetBundles/AppleOnDemandResources/Build/CustomAssetPackContent' folder.
                        // The OnDemandResourcesBuildProcessor moves bundles assigned to "fast-follow" or "on-demand" asset packs to this location
    ...
    /// <summary>
    /// Contains settings for <see cref="OnDemandResourcesInitialization"/>.
    /// </summary>
    [Serializable]
    public class OnDemandResourcesInitializationData
    {
        [SerializeField]
